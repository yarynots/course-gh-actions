...
resource "azurerm_service_plan" "web-api" {
  ...
  os_type  = "Linux"
  sku_name = "B1" # F1 - Free, B1 - Basic, P1 - Premium
}

resource "azurerm_linux_web_app" "web-api" {
  # az webapp list --output table
  # https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/linux_web_app
  name                = "gh-actions-web-api-yarynots" # => https://gh-actions-web-api.azurewebsites.net # *** MAKE SURE TO SET NEW NAME (one universal namespace for everyone) ***
  resource_group_name = azurerm_resource_group.gh-actions.name
  location            = azurerm_resource_group.gh-actions.location
